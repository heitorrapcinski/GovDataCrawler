...
- [ ] 1. Scaffold project structure and configure dependencies
  - [ ] 1.1 Create the Python package directory `gov_data_crawler/` with `__init__.py` and `__main__.py`
  - [ ] 1.2 Create `pyproject.toml` with project metadata, Python 3.10+ requirement, and pinned dependencies: `requests==2.32.3`, `beautifulsoup4==4.12.3`, `lxml==5.3.1`
  - [ ] 1.3 Add dev dependencies: `pytest==8.3.5`, `hypothesis==6.122.3`, `pytest-cov==6.1.1`, `responses==0.25.7`
  - [ ] 1.4 Create the test directory structure: `tests/unit/`, `tests/property/`, `tests/integration/` with `__init__.py` files
  - [ ] 1.5 Create `.gitignore` with entries for `__pycache__/`, `*.pyc`, `.pytest_cache/`, `htmlcov/`, `*.egg-info/`, `dist/`, `build/`, `target/`, `.env`, `venv/`, `.venv/`
  - [ ] 1.6 Create `.dockerignore` with entries for `.git/`, `.kiro/`, `__pycache__/`, `*.pyc`, `.pytest_cache/`, `htmlcov/`, `target/`, `tests/`, `*.log`, `.env`, `venv/`, `.venv/`
  - [ ] 1.7 Verify the project installs and pytest discovers the test directories
